"""Soccer 도메인: Exaone이 팀(teams) 엠베딩 생성용 Python 스크립트 코드를 생성한다.

Cursor 터미널에서 (반드시 프로젝트 루트에서 실행):
  1) python app/exaone/soccer_team_embeddings.py   → Exaone이 코드 생성, scripts/team_embedding_run.py 저장
  2) python scripts/team_embedding_run.py          → 생성된 스크립트로 실제 엠베딩 실행
"""
...
# 프로젝트 루트 (app/exaone/soccer_team_embeddings.py 기준 2단계 상위)
...
# Exaone이 생성한 팀 엠베딩 스크립트 저장 경로 (프로젝트 루트/scripts/team_embedding_run.py)
OUTPUT_SCRIPT = ROOT / "scripts" / "team_embedding_run.py"


def build_prompt() -> str:
    """팀 JSONL을 기반으로 Exaone이 전체 팀 임베딩 스크립트를 생성하도록 유도."""
...
2. Read JSONL from {data_root}/teams.jsonl (open file, for each non-empty line: json.loads(line) to a list of dicts)
3. For each team record dict, build one string like "id: <id> | team_name: <team_name> | region_name: <region_name> | stadium_id: <stadium_id> | ..." for embedding
...
5. Create OUTPUT_DIR if needed, write {data_root}/embeddings/teams_embeddings.jsonl: each line is json.dumps({{"id": team_id, "team_name": team_name, "embedding": list_of_floats}})
...
def main() -> None:
    ...
    print("[ExaOne] Team 엠베딩 스크립트 코드 생성 중...")
    messages = [
        {
            "role": "system",
    ...
    print(f"         cd {{ROOT}}")
    print("         python scripts/team_embedding_run.py")
