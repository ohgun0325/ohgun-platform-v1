...
output_file = "app/domain/v10/soccer/models/bases/team_embeddings.py"

# team.py 파일 읽기
team_file = Path("app/domain/v10/soccer/models/bases/team.py")
team_content = team_file.read_text(encoding="utf-8") if team_file.exists() else "# Team ORM (베이스 모델)\n# team_id, team_name, region_name, stadium_id 등"

# 프롬프트 작성
prompt = f"""다음 SQLAlchemy Team 모델을 참고하여 TeamEmbedding ORM 클래스를 작성하세요.

=== Team 모델 코드 ===
{team_content}

=== Alembic 마이그레이션 테이블 스키마 ===
테이블명: teams_embeddings
컬럼:
- id: BigInteger, PK, autoincrement=True, nullable=False, comment='임베딩 레코드 고유 식별자'
- team_id: BigInteger, FK -> teams.id, nullable=False, ondelete='CASCADE', comment='팀 ID'
...
5. relationship: team (back_populates="embeddings") 설정
6. team.py의 코딩 스타일과 일관성 유지 (주석 형식, Column 정의 방식 등)
7. 모든 Column에 comment 추가
8. __tablename__ = "teams_embeddings" 사용
9. Python 코드만 출력 (주석이나 설명 없이 순수 코드만)
10. docstring은 Team 모델과 유사한 형식으로 작성
