...
output_file = "app/domain/v10/soccer/models/bases/stadium_embeddings.py"

# stadium.py 파일 읽기
stadium_file = Path("app/domain/v10/soccer/models/bases/stadium.py")
stadium_content = stadium_file.read_text(encoding="utf-8") if stadium_file.exists() else "# Stadium ORM (베이스 모델)\n# stadium_id, stadium_name, hometeam_id, seat_count, address 등"

# 프롬프트 작성
prompt = f"""다음 SQLAlchemy Stadium 모델을 참고하여 StadiumEmbedding ORM 클래스를 작성하세요.

=== Stadium 모델 코드 ===
{stadium_content}

=== Alembic 마이그레이션 테이블 스키마 ===
테이블명: stadiums_embeddings
컬럼:
- id: BigInteger, PK, autoincrement=True, nullable=False, comment='임베딩 레코드 고유 식별자'
- stadium_id: BigInteger, FK -> stadiums.id, nullable=False, ondelete='CASCADE', comment='경기장 ID'
...
5. relationship: stadium (back_populates="embeddings") 설정
6. stadium.py의 코딩 스타일과 일관성 유지 (주석 형식, Column 정의 방식 등)
7. 모든 Column에 comment 추가
8. __tablename__ = "stadiums_embeddings" 사용
9. Python 코드만 출력 (주석이나 설명 없이 순수 코드만)
10. docstring은 Stadium 모델과 유사한 형식으로 작성
