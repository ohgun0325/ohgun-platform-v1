...
output_file = "app/domain/v10/soccer/models/bases/schedule_embeddings.py"

# schedules.py 파일 읽기
schedules_file = Path("app/domain/v10/soccer/models/bases/schedules.py")
schedules_content = schedules_file.read_text(encoding="utf-8")

# 프롬프트 작성
prompt = f"""다음 SQLAlchemy Schedule 모델을 참고하여 ScheduleEmbedding ORM 클래스를 작성하세요.

=== Schedule 모델 코드 ===
{schedules_content}

=== Alembic 마이그레이션 테이블 스키마 ===
테이블명: schedules_embeddings
컬럼:
- id: BigInteger, PK, autoincrement=True, nullable=False, comment='임베딩 레코드 고유 식별자'
- schedule_id: BigInteger, FK -> schedules.id, nullable=False, ondelete='CASCADE', comment='경기 일정 ID'
...
5. relationship: schedule (back_populates="embeddings") 설정
6. schedules.py의 코딩 스타일과 일관성 유지 (주석 형식, Column 정의 방식 등)
7. 모든 Column에 comment 추가
8. __tablename__ = "schedules_embeddings" 사용
9. Python 코드만 출력 (주석이나 설명 없이 순수 코드만)
10. docstring은 Schedule 모델과 유사한 형식으로 작성
