"""경기 일정 업로드 파이프라인 오케스트레이션 모듈.

현재는 규칙 기반(ScheduleService)만 사용하지만,
...
from app.domain.soccer.services.schedule_service import ScheduleService


class ScheduleOrchestrator:
    """경기 일정 업로드에 대한 전략 선택 및 실행을 담당하는 오케스트레이터.

    - 현재는 규칙 기반(ScheduleService) 하나만 사용
    - 그러나 PlayerStrategyType/StrategyDecision 을 재사용하여
      응답 스키마와 메타데이터 구조를 player 파이프라인과 동일하게 유지
    """

    def __init__(self, service: Optional[ScheduleService] = None) -> None:
        self._service = service or ScheduleService()

    async def route_schedules(
        self,
        ...
        decision = StrategyDecision(
            ...
            raw_output={"reason": "Schedule 도메인은 현재 규칙 기반 처리로 고정"},
        )
