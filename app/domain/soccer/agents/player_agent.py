"""선수 데이터에 대한 정책 기반(LLM 에이전트) 처리 로직.
...
class PlayerAgent:
    ...
    LLM을 활용하여 업로드된 선수 데이터를 해석/요약하거나,
    ...
    def process(
        ...
        system_prompt = (
            "당신은 축구 선수 데이터 품질을 점검하고, 정책 기반(룰 기반이 아닌) 모델링이 "
            "적합한지 판단하고 요약을 작성하는 데이터 분석가입니다."
        )
        user_prompt = (
            f"다음은 '{data_type}' 타입의 선수 데이터 예시입니다 (파일: {file_name}).\n\n"
            f"{preview_text}\n\n"
