...
class SQLRecordManager(RecordManager):
    ...
    def create_schema(self) -> None:
        ...
        # NOTE:
        #   원본 구현에서는 여기서 Base.metadata.create_all(self.engine)을 호출하여
        #   모든 테이블을 직접 생성합니다.
        #   이 프로젝트에서는 Alembic 마이그레이션으로 스키마/온톨로지를 통제하기 위해
        #   metadata.create_all() 호출을 비활성화했습니다.
        #   스키마가 없으면 Alembic을 통해 먼저 생성해야 합니다.
        raise RuntimeError(
            "create_schema()는 비활성화되었습니다. "
            "데이터베이스 스키마는 Alembic 마이그레이션으로 관리하세요."
        )
    ...
    async def acreate_schema(self) -> None:
        ...
        # async 버전도 동일한 이유로 비활성화
        raise RuntimeError(
            "acreate_schema()는 비활성화되었습니다. "
            "데이터베이스 스키마는 Alembic 마이그레이션으로 관리하세요."
        )
